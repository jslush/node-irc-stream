...
function Client(c) {
  var self = this;
  // If called as a function, return a new Client.
  ...
  self.middleware = {
    'output': [],
    'input': [],
    'defaults': ['flood-protection', 'ctcp', 'channel']
  };
...
Client.prototype.loadMiddleware = function (middleware, config) {
  try {
    ...
    // Or a string that is recognized as a built-in middleware.
      ...
      if (path.basename(middleware) !== middleware) {
        throw new Error('Name of a built-in middleware can\'t contain directory separators');
      }
      return require(path.normalize(__dirname + '/middleware/' + middleware)).call(this, config);
    }
  } catch (e) {
    e.message = 'Could not load middleware: "' + middleware + '"!\n' + e.message;
    throw e;
  }
};
...
Client.prototype.use = function (middleware, config) {
  ...
  // TODO: Should we do something prettier here if loadMiddleware throws?
